...
tablename_etl='cashout_posting_rekap'
...
# Contoh query
    query = f"""
    SELECT date(entry_time)entry_time,latitude,longitude,account_no,
    COUNT(posting_id)posting_id,
    SUM(amount)amount,
    SUM(fee_amount)fee_amount
    FROM pospayDB.cashout_posting
    ...
    gdf_joined['product_code']='CASHOUT'
    gdf_joined['product_name']='CASHOUT'
