...
# Contoh query
    query = f"""
    SELECT date(entry_time)entry_time,product_code,latitude,longitude,
    COUNT(posting_id)posting_id,
    COUNT(DISTINCT account_no)account_no,
    SUM(amount)amount,
    ...
    gdf_joined[['posting_id','account_no','amount','fee_amount']]=gdf_joined[['posting_id','account_no','amount','fee_amount']].astype(int)
    ...
    gdf_joined=gdf_joined.groupby(['entry_time','product_code','name','regional','kc','kcu','jenis'],dropna=False).agg(
    {
        'posting_id':'sum',
        'account_no':'sum',
        'amount':'sum',
