...
tablename_etl='biller_posting_rekap'
...
t1='biller_product'
t2='biller_product_backup_20240401'
t3='biller_product_duplikat'
t4='biller_product_pbb'
t4='biller_product_pbb_to'
t5='biller_product_to'
...
FROM {t5}
"""
df_ref_produk=pd.read_sql(query,engine)

df_ref_produk.drop_duplicates(subset='code',keep='first',inplace=True)

# %%
df_ref_produk['name']=df_ref_produk['name'].str.strip()
df_ref_produk['name']=df_ref_produk['name'].str.upper()
...
# Contoh query
    query = f"""
    SELECT date(entry_time)entry_time,product_code,latitude,longitude, account_no,
    COUNT(posting_id)posting_id,
    SUM(amount)amount,
    SUM(fee_amount)fee_amount
    FROM pospayDB.biller_posting
    ...
    gdf_joined[['posting_id','amount','fee_amount']]=gdf_joined[['posting_id','amount','fee_amount']].astype(int)

    gdf_joined=pd.merge(gdf_joined,df_ref_produk,left_on='product_code',right_on='code',how='left')
    gdf_joined.drop(columns='code',inplace=True)

    gdf_joined = gdf_joined.rename(columns={'name': 'product_name'})

    gdf_joined.loc[gdf_joined['kdcpum'].isna(),['kdcpum','regional','kc','kcu','jenis']]='TIDAK DITEMUKAN'
    gdf_joined=gdf_joined.groupby(['entry_time','product_code','product_name','regional','kc','kcu','jenis','account_no'],dropna=False).agg(
    {
        'posting_id':'sum',
        'amount':'sum',
        'fee_amount':'sum'
