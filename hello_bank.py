...
tablename_etl='bank_posting_rekap'
...
# tanggal_awal=pd.Timestamp(2020,1,1).date()
...
# lokasi_data=r'C:\Users\User\Documents\clustering_bandung\notebook\wilayah\Batas_Wilayah_KelurahanDesa_10K_AR.shp'
...
# %%
# Pastikan shapefile juga di EPSG:4326, kalau beda â†’ konversi
...
query_ref_produk = f"""
SELECT DISTINCT code,upper(name)product_name
FROM pospayDB.bank_product_backup_20240429
UNION
SELECT DISTINCT code,upper(name)product_name
FROM pospayDB.bank_product
    """

# Eksekusi query langsung ke DataFrame Pandass
df_ref_produk = pd.read_sql(query_ref_produk, engine)
...
# Contoh query
    query = f"""
    SELECT date(entry_time)entry_time,bank_code product_code,latitude,longitude,account_no,
    COUNT(posting_id)posting_id,
    SUM(amount)amount,
    SUM(fee_amount)fee_amount
    FROM pospayDB.bank_posting
